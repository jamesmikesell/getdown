 ...
 */
public class RegularResource implements Resource
{
    /**
     * Creates a resource with the supplied remote URL and local path.
     */
    public RegularResource (String path, URL remote, File local, boolean unpack)
    {
    	_path = path;
        _remote = remote;
        _local = local;
        ...
        if(isPacked200Jar(lpath))
        	_compressionType = CompressionType.Packed200Jar;
        
        if (_unpack && _compressionType == CompressionType.Jar) {
            _unpacked = _local.getParentFile();
        } else if(_unpack && _compressionType == CompressionType.Packed200Jar) {
    ...
    {
        if (other instanceof RegularResource) {
            return _path.equals(((RegularResource)other)._path);
        } else {
            return false;
        }
    }
